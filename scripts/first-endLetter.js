...
export const firsendLetter = () => {
    ...
    if (firstIndex !== -1) {
        mensaje.innerHTML = `La primera letra 'A' de tu nombre está en la posición ${firstIndex}.`;
    } else {
        mensaje.innerHTML = `No se encontró la letra 'A' en tu nombre.`;
    }

    if (lastIndex !== -1) {
        mensaje.innerHTML += `<p>La última letra 'A' de tu nombre está en la posición ${lastIndex}.</p>`;
    } else {
    ...
    if (nombreCompleto.length >= 3) {
        ...
        mensaje.innerHTML += `<p>Tu nombre sin las tres primeras letras es: <strong>${nuevoNombre}</strong></p>`;
